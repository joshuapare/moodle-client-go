/*
Moodle Webservice API

Auto-generated OpenAPI spec for Moodle's Webservice API. Requires installation of the Catalyst RESTful API plugin: https://github.com/catalyst/moodle-webservice_restful.

API version: 4.3.4
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package moodleclient

import (
	"encoding/json"
)

// checks if the CoreCourseGetCategoriesRequestCriteriaInner type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CoreCourseGetCategoriesRequestCriteriaInner{}

// CoreCourseGetCategoriesRequestCriteriaInner struct for CoreCourseGetCategoriesRequestCriteriaInner
type CoreCourseGetCategoriesRequestCriteriaInner struct {
	// The category column to search, expected keys (value format) are:\"id\" (int) the category id,\"ids\" (string) category ids separated by commas,\"name\" (string) the category name,\"parent\" (int) the parent category id,\"idnumber\" (string) category idnumber - user must have 'moodle/category:manage' to search on idnumber,\"visible\" (int) whether the returned categories must be visible or hidden. If the key is not passed,                                              then the function return all categories that the user can see. - user must have 'moodle/category:manage' or 'moodle/category:viewhiddencategories' to search on visible,\"theme\" (string) only return the categories having this theme - user must have 'moodle/category:manage' to search on theme
	Key *string `json:"key,omitempty"`
	// the value to match
	Value *string `json:"value,omitempty"`
}

// NewCoreCourseGetCategoriesRequestCriteriaInner instantiates a new CoreCourseGetCategoriesRequestCriteriaInner object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCoreCourseGetCategoriesRequestCriteriaInner() *CoreCourseGetCategoriesRequestCriteriaInner {
	this := CoreCourseGetCategoriesRequestCriteriaInner{}
	var key string = "null"
	this.Key = &key
	var value string = "null"
	this.Value = &value
	return &this
}

// NewCoreCourseGetCategoriesRequestCriteriaInnerWithDefaults instantiates a new CoreCourseGetCategoriesRequestCriteriaInner object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCoreCourseGetCategoriesRequestCriteriaInnerWithDefaults() *CoreCourseGetCategoriesRequestCriteriaInner {
	this := CoreCourseGetCategoriesRequestCriteriaInner{}
	var key string = "null"
	this.Key = &key
	var value string = "null"
	this.Value = &value
	return &this
}

// GetKey returns the Key field value if set, zero value otherwise.
func (o *CoreCourseGetCategoriesRequestCriteriaInner) GetKey() string {
	if o == nil || IsNil(o.Key) {
		var ret string
		return ret
	}
	return *o.Key
}

// GetKeyOk returns a tuple with the Key field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CoreCourseGetCategoriesRequestCriteriaInner) GetKeyOk() (*string, bool) {
	if o == nil || IsNil(o.Key) {
		return nil, false
	}
	return o.Key, true
}

// HasKey returns a boolean if a field has been set.
func (o *CoreCourseGetCategoriesRequestCriteriaInner) HasKey() bool {
	if o != nil && !IsNil(o.Key) {
		return true
	}

	return false
}

// SetKey gets a reference to the given string and assigns it to the Key field.
func (o *CoreCourseGetCategoriesRequestCriteriaInner) SetKey(v string) {
	o.Key = &v
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *CoreCourseGetCategoriesRequestCriteriaInner) GetValue() string {
	if o == nil || IsNil(o.Value) {
		var ret string
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CoreCourseGetCategoriesRequestCriteriaInner) GetValueOk() (*string, bool) {
	if o == nil || IsNil(o.Value) {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *CoreCourseGetCategoriesRequestCriteriaInner) HasValue() bool {
	if o != nil && !IsNil(o.Value) {
		return true
	}

	return false
}

// SetValue gets a reference to the given string and assigns it to the Value field.
func (o *CoreCourseGetCategoriesRequestCriteriaInner) SetValue(v string) {
	o.Value = &v
}

func (o CoreCourseGetCategoriesRequestCriteriaInner) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CoreCourseGetCategoriesRequestCriteriaInner) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Key) {
		toSerialize["key"] = o.Key
	}
	if !IsNil(o.Value) {
		toSerialize["value"] = o.Value
	}
	return toSerialize, nil
}

type NullableCoreCourseGetCategoriesRequestCriteriaInner struct {
	value *CoreCourseGetCategoriesRequestCriteriaInner
	isSet bool
}

func (v NullableCoreCourseGetCategoriesRequestCriteriaInner) Get() *CoreCourseGetCategoriesRequestCriteriaInner {
	return v.value
}

func (v *NullableCoreCourseGetCategoriesRequestCriteriaInner) Set(val *CoreCourseGetCategoriesRequestCriteriaInner) {
	v.value = val
	v.isSet = true
}

func (v NullableCoreCourseGetCategoriesRequestCriteriaInner) IsSet() bool {
	return v.isSet
}

func (v *NullableCoreCourseGetCategoriesRequestCriteriaInner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCoreCourseGetCategoriesRequestCriteriaInner(val *CoreCourseGetCategoriesRequestCriteriaInner) *NullableCoreCourseGetCategoriesRequestCriteriaInner {
	return &NullableCoreCourseGetCategoriesRequestCriteriaInner{value: val, isSet: true}
}

func (v NullableCoreCourseGetCategoriesRequestCriteriaInner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCoreCourseGetCategoriesRequestCriteriaInner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


